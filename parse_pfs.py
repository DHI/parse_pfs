...
class utils:
    def __init__(self):
        self.units = {"Eddy": "m²/s",
                 ...
                 "Distribution factor": "",
                 "Settling velocity coefficient": "m/s",
                 "Settling velocity": "m/s",
                 "Critical shear stress": "N/m²",
                 "Erosion coefficient": "kg/m²/s",
                 "Bed roughness": "m",
                 "Sediment transition rate": "kg/m²/s",
                 }
    
    def _parse_format(self, data, variable, key="constant_value", has_soft_time=False, unit=None):
        if unit is None:
        ...
        if data["format"] == 0:
            ...
            output[f"{variable} value"] = f"{data[key]} {unit}"
        if data["format"] == 2:
...
class PFS_Water_Column(utils):
    def __init__(self, WC, fractions, layers):
        super().__init__()
        self.WC = WC
        self.fractions = fractions
        self.layers = layers
        self.n_fractions = len(self.fractions.keys())
        self.n_layers = len(self.layers.keys())
        self.n_sand_fractions = 0
        self._parse_sand_fractions()
        self.settling_velocity = self._parse_settling_velocity()
        self.deposition = self._parse_deposition()
        self.viscosity_density = self._parse_viscosity_density()
        self.attributes = {"Settling velocity": self.settling_velocity,
                           "Deposition": self.deposition,
                           "Viscosity and density": self.viscosity_density}

    def _parse_sand_fractions(self):
        sand = self.WC["SAND_FRACTIONS"]
        if sand["sand_fraction_description"] == 0:
            return "No sand fraction"
        for i in range(self.n_fractions):
            fraction = sand[f"FRACTION_{i+1}"]
            if not fraction["include_sand"]:
                self.fractions[f"Fraction {i+1}"]["Type"] = "Suspended"
                continue
            self.n_sand_fractions += 1
            self.fractions[f"Fraction {i+1}"]["Type"] = "Sand"
            if sand["sand_fraction_description"] == 1:
                self.fractions[f"Fraction {i+1}"]["Mean settling velocity"] = fraction["mean_settling_velocity"]                

    def _parse_settling_velocity(self):
        data = self.WC["SETTLING_VELOCITY"]
        if self.n_sand_fractions == self.n_fractions:
            return "No settling velocity calculation. All fractions are sand"
        output = {"Density of sediment": str(data["rho_sediment"]) + " kg/m³",
                  "Concentration for fluctuation": str(data["cfloc"]) + " kg/m³",
                  "Concentration for hindered settling": str(data["chinder"]) + " kg/m³"}
        if data["hindered_settling_description"] in [1, 2]:
            output.update({"Gelling point": str(data["cgel"]) + " kg/m³",
                           "Hindered settlin calculations": "Using Winterwerp formulation" if data["hindered_settling_description"] == 1 else "Using Richardson and Zaki formulation"})
        for i in range(self.n_fractions):
            if self.fractions[f"Fraction {i+1}"]["Type"] == "Suspended":
                fraction = data[f"FRACTION_{i+1}"]
                flocculation = fraction["type_flocculation"]
                self.fractions[f"Fraction {i+1}"]["Flocculation"] = "Included" if flocculation == 1 else "Not included"
                if flocculation == 1:
                    self.fractions[f"Fraction {i+1}"]["Settling velocity coefficient"] = self._parse_format(fraction["SETTLING_VELOCITY_COEFFICIENT"], variable="Settling velocity coefficient")
                else:
                    self.fractions[f"Fraction {i+1}"]["Settling velocity"] = self._parse_format(fraction["CONSTANT_SETTLING_VELOCITY"], variable="Settling velocity")
        return output

    def _parse_deposition(self):
        deposition = self.WC["DEPOSITION"]
        output = {"Concnetration profile": "Rouse profile" if deposition["concentration_profile"] == 0 else "Teeter profile"}
        if deposition["concentration_profile"] == 0:
            output.update({"Relative centroid height": str(deposition["centroid_height"]) + " m"})
        for i in range(self.n_fractions):
            fraction = deposition[f"FRACTION_{i+1}"]
            self.fractions[f"Fraction {i+1}"]["Deposition"] = self._parse_format(fraction["CRITICAL_SHEARSTRESS_FOR_DEPOSITION"], variable="Critical shear stress")
        return output

    def _parse_viscosity_density(self):
        visocisty_density = self.WC["DENSITY_AND_VISCOSITY"]
        if not visocisty_density["include_density_feedback_on_HD"]:
            return "Not included"
        return {"Density of suspended sediment": str(visocisty_density["density_of_sediment"]) + " kg/m³",
                "Base in viscosity formulation": visocisty_density["base_in_viscosity_formula"],
                "Concnetration in viscosity formulation": str(visocisty_density["concentration_in_viscosity_formula"]) + " kg/m³"}

class PFS_MT_Bed(utils):
    def __init__(self, bed, fractions, layers):
        super().__init__()
        self.bed = bed
        self.fractions = fractions
        self.layers = layers
        self.n_fractions = len(self.fractions.keys())
        self.n_layers = len(self.layers.keys())
        self.erosion = self._parse_erosion()
        self._parse_bed_density()
        self.bed_roughness = self._parse_format(bed["ROUGHNESS"], variable="Bed roughness")
        self.transition = self._parse_transition()
        
        self.attributes = {"Erosion": self.erosion,
                           **self.bed_roughness,}
        if self.transition is not None:
            self.attributes.update(self.transition)

    def _parse_erosion(self):
        erosion = self.bed["EROSION"]
        erosion_descriptions = {0: "Hard mud", 1:"Soft mud", 2: "Mass erosion"}
        output = {"Maximum concentration allowed due to erosion": str(erosion["max_concentration_due_to_erosion"]) + " kg/m³",
                "Type of critical shear stress": "One value for all fractions in each layer" if erosion["type_of_critical_shearstress_for_erosion"] == 1 else "Individual value for each fraction in each layer"}
        for i in range(self.n_layers):
            layer = erosion[f"LAYER_{i+1}"]
            layer_erosion = {"Erosion description": erosion_descriptions[layer["description"]]}
            if layer["description"] == 0:
                layer_erosion.update({"Alpha coefficient": layer["POWER_OF_EROSION"]["constant_value"]})
            elif layer["description"] == 1:
                layer_erosion.update({"Power of erosion": layer["POWER_OF_EROSION"]["constant_value"]}) 
            if layer["description"] != 2:
                layer_erosion.update(**self._parse_format(layer["EROSION_COEFFICIENT"], variable="Erosion coefficient"))
            crtitical_shear_stress = layer["CRITICAL_SHEARSTRESS_FOR_EROSION"]["constant_values"]
            if self.n_fractions > 1:
                layer_erosion.update({"Critical shear stress for each fraction": {f"Fraction {j+1}": crtitical_shear_stress[j] for j in range(self.n_fractions)}})
            else:
                layer_erosion.update({"Critical shear stress": crtitical_shear_stress})
            self.layers[f"Layer {i+1}"]["Erosion"] = layer_erosion
        return output

    def _parse_bed_density(self):
        density = self.bed["BED_DENSITY"]
        for i in range(self.n_layers):
            layer = density[f"LAYER_{i+1}"]
            self.layers[f"Layer {i+1}"]["Density of bed layer"] = self._parse_format(layer, variable="Density")

    def _parse_transition(self):
        transition = self.bed["TRANSITION"]
        if not transition["include_transition"]:
            return {"Transition between layers": "Not included"}
        else:
            for i in range(self.n_layers-1):
                layer = transition[f"LAYER_{i+1}"]
                self.layers[f"Layer {i+1}"][f"Transition between Layer {i+1} and {i+2}"] = self._parse_format(layer, variable="Sediment transition rate")
            return None

class PFS_MT_Dredger(utils):
    def __init__(self, dredgers_info, fractions, layers):
        super().__init__()
        self.dredgers_info = dredgers_info
        self.fractions = fractions
        self.layers = layers
        self.n_fraction = len(self.fractions.keys())
        self.n_layers = len(self.layers.keys())
        self.n_dredgers = dredgers_info["number_of_dredgers"]
        if self.n_dredgers == 0:
            self.dredgers = "No dredgers"
        else:
            self._parse_dredgers()
        if self.n_dredgers > 0:
            if self.dredgers_info["update_bed"]:
                self.attributes = {"Dredgers": self.dredgers,
                                   "Update of bed": "Included"}
            else:
                self.attributes = {"Dredgers": self.dredgers,
                                   "Update of bed": "Not included"}
                

    def _parse_dredgers(self):
        n_included = 0
        self.dredgers = {}
        for i in range(self.n_dredgers):
            dredger = self.dredgers_info[f"DREDGER_{i+1}"]
            if dredger["include"] == 0:
                continue
            n_included += 1
            name = dredger["name"]
            types = {1: "Spill in the bottom layer",
                     2: "Spill in the top layer",
                     3: "Spill in the bottom and the top layer",
                     4: "Spill distributed in the water column",
                     5: "Spill distributed over a specified distance from the bottom",
                     6: "Spill in specified distance from the bottom",
                     7: "Spill distributed over a specified distance from the surface",
                     8: "Spill in specified distance from the surface",
                     9: "Spill in specified layer number"}
            output = {"Type of spilling": types[dredger["type"]]}
            if dredger["type"] in [5, 6]:
                output.update({"Distance from the bottom": str(dredger["distance_above_bed"]) + " m"})
            if dredger["type"] in [7, 8]:
                output.update({"Distance from the surface": str(dredger["distance_below_surface"]) + " m"})
            if dredger["type"] == 9:
                output.update({"Layer number": dredger["layer_no"]})
            if dredger["update_dredger"]:
                output.update({"Update of the dredger": "Included",
                               "Initial mass on dredger": str(dredger["initial_dredger_mass"]) + " t",
                               "Maximum mass on dredger": str(dredger["maximum_dredger_mass"]) + " t"})
            if dredger["format_weight"] == 0:
                output.update({"Distribution of weight of each fraction": {f"Fraction {i+1}": str(np.round(dredger["weights_fraction"][i], 2)) + "%" for i in range(self.n_fraction)}})
            self.dredgers[name] = output
            n_included += 1
        if n_included == 0:
            self.dredgers = "No dredgers"

class PFS_MT_Disposal(utils):
    def __init__(self, disposal, fractions):
        super().__init__()
        self.disposal = disposal
        self.fractions = fractions
        self.n_fractions = len(self.fractions.keys())
        self.n_vessels = disposal["number_of_vessels"]
        if self.n_vessels == 0:
            self.vessels = "No vessels"
        else:
            self._parse_vessels()
        if self.n_vessels > 0:
            if self.disposal["stripping"] == 1:
                self.attributes = {"Vessels": self.vessels,
                                   "Stripping": "Included",
                                   "Convective descent coefficient": disposal["c_strip1"],
                                   "Collapse surge coefficient": disposal["c_strip2"]}
            else:
                self.attributes = {"Vessels": self.vessels,
                                   "Stripping": "Not included"}

    def _parse_vessels(self):
        n_included = 0
        self.vessels = {}
        for i in range(self.n_vessels):
            vessel = self.disposal[f"VESSEL_{i+1}"]
            if vessel["include"] == 0:
                continue
            n_included += 1
            name = vessel["name"]
            output = {"Overall area": str(vessel["area_overall"]) + " m²",
                      "Opening area": str(vessel["area_opening"]) + " m²",
                      "Inner area": str(vessel["area_inner"]) + " m²",
                      "Unloaded draft": str(vessel["draft_unloaded"]) + " m"}
            if vessel["weight_format"] == 0:
                output.update({"Distribution of weight of each fraction": {f"Fraction {i+1}": str(np.round(vessel["weights_fraction"][i], 2)) + "%" for i in range(self.n_fractions)}})
            self.vessels[name] = output
            n_included += 1
        if n_included == 0:
            self.vessels = "No vessels"

class PFS_Mud_Transport(utils):
    def __init__(self, MT, boundaries, sources):
        super().__init__()
        self.MT = MT
        self.boundaries = boundaries
        self.sources = sources
        self.n_layers = 0
        self.n_fractions = 0
        self.solution_technique = PFS_Solution_Technique(MT["SOLUTION_TECHNIQUE"], module="salinity_temperature").attributes
        self.parameters = self._parse_parameters()
        self.fractions = {f"Fraction {i+1}": {} for i in range(self.n_fractions)}
        self.layers = {f"Layer {i+1}": {} for i in range(self.n_layers)}
        tmp = PFS_Water_Column(MT["WATER_COLUMN"], self.fractions, self.layers)
        
        self.water_column = tmp.attributes
        self.fractions = tmp.fractions
        self.layers = tmp.layers
        tmp = PFS_MT_Bed(MT["BED"], self.fractions, self.layers)
        self.bed = tmp.attributes
        self.fractions = tmp.fractions
        self.layers = tmp.layers
        self.wave_forcing = self._parse_wave_forcing()
        tmp = PFS_MT_Dredger(MT["DREDGING"], self.fractions, self.layers)
        self.dredgers = tmp.dredgers
        if isinstance(self.dredgers, dict):
            self.dredging = tmp.attributes
        else:
            self.dredging = self.dredgers
        tmp = PFS_MT_Disposal(MT["DISPOSAL"], self.fractions)
        self.vessels = tmp.vessels
        if isinstance(self.vessels, dict):
            self.disposal = tmp.attributes
        else:
            self.disposal = self.vessels
        self._parse_dispersion()
        self._parse_sources()
        initial_conditions = self._parse_initial_condition()
        self._parse_boundaries()
        self.morphology = self._parse_morphology()
        self.attributes = {**self.parameters,
                           "Solution technique": self.solution_technique,
                           "Water column properties": self.water_column,
                           "Bed parameters": self.bed,
                           "Wave forcing": self.wave_forcing,
    ...
    def _parse_parameters(self):
        model = self.MT["MODEL_DEFINITION"]
        self.n_layers = model["number_of_layers"]
        self.n_fractions = model["number_of_fractions"]
        return {"Number of layers": self.n_layers,
                "Number of fractions": self.n_fractions}
    
    def _parse_wave_forcing(self):
        waves = self.MT["FORCINGS"]["WAVES"]
        if waves["type"] == 0:
            return "Not included"
        wave_field = waves["WAVE_FIELD"]
        output = {}
        if wave_field["format"] == 0:
            
            output.update({"Wave field format": "Constant wave field",
                           "Wave height": str(wave_field["wave_height"]) + " m",
                           periods[wave_field["type_of_wave_period"]] + "Wave period":  str(wave_field["wave_period"]) + " s",
                           "Wave direction": str(wave_field["wave_direction"]) + " degrees"})
        elif wave_field["format"] == 1:
            output.update({"Wave field format": "Varying in time, constant in space",
                           "Soft start interval": str(wave_field["soft_time_interval"]) + " s"})
        elif wave_field["format"] == 3:
            output.update({"Wave field format": "Varying in time and space",
                           "Soft start interval": str(wave_field["soft_time_interval"]) + " s"})
        correction_types = {0: "Not included", 1: "Correction of wave heights", 2: "Correction of wave heights and wave periods"}
        output.update({"Wave correction": correction_types[waves["type_of_wave_height_correction"]]})
        if waves["type_of_wave_height_correction"] != 0:
            output.update({"Wave height/depth limit": waves["wave_height_over_depth_limit"],
                           "Minimum water depth": waves["minimum_waterdepth_for_including_waves"]})
        output.update({"Liquifaction": "Not included" if waves["include_liquefaction"] == 0 else "Included with liquifaction factor of " + str(waves["liquefaction_factor"])})
        shear_stresses = {0: "Mean (Soulsby et al. 1993)", 1: "RMS (Soulsby et al. 1993)", 2: "Max (Fredsoe 1981)", 3: "Max (Soulsby et al. 1993)"}
        output.update({"Shear stress formulation": shear_stresses[waves["shearstress_formulation"]]})
        periods = {1: "Peak ", 2: "Mean "}
        
        
        return output
            
    def _parse_dispersion(self):
        dispersion = self.MT["DISPERSION"]
        keys = {"HORIZONTAL_DISPERSION": "Horizontal dispersion", "VERTICAL_DISPERSION": "Vertical dispersion"}
        for key in keys.keys():
            info = dispersion[key]
            for i in range(self.n_fractions):
                fraction = info[f"SSC_FRACTION_{i+1}"]
                if fraction["type"] == 0:
                    self.fractions[f"Fraction {i+1}"][keys[key]] = "Not included"
                elif fraction["type"] == 1:
                    self.fractions[f"Fraction {i+1}"][keys[key]] = {"Formulation": "Dispersion coefficient",
                                                                    **self._parse_format(fraction["DISPERSION_COEFFICIENT"], variable="Dispersion coefficient", key="constant_value")}
                elif fraction["type"] == 2:
                    self.fractions[f"Fraction {i+1}"][keys[key]] = {"Formulation": "Scaled eddy viscosity",
                                                                    **self._parse_format(fraction["SCALED_EDDY_VISCOSITY"], variable="Scaled eddy viscosity", key="sigma")}
                    
    def _parse_sources(self):
        sources = self.MT["SOURCES"]
        for i in range(self.sources["Number of sources"]):
            source = sources[f"SOURCE_{i+1}"]
            if self.n_fractions > 0:
                self.sources[f"Source {i+1}"]["Mud transport information"] = {}
            for f in range(self.n_fractions):
                fraction = source[f"SSC_FRACTION_{f+1}"]
                if fraction["type_of_component"] == 0:
                    self.sources[f"Source {i+1}"]["Mud transport information"][f"Fraction {f+1}"] = {"Type": "Specified concentration",
                                                                        **self._parse_format(fraction, variable="Concentration", unit="kg/m³")}
                elif fraction["type_of_component"] == 1:
                    self.sources[f"Source {i+1}"]["Mud transport information"][f"Fraction {f+1}"] = {"Type": "Excess concentration",
                                                                        **self._parse_format(fraction, variable="Concentration", unit="kg/m³")}

    def _parse_initial_condition(self):
        initial = self.MT["INITIAL_CONDITIONS"]
        output = {"Fraction concentrations": {}, "Layer specifications": {}}
        for i in range(self.n_fractions):
            fraction = initial[f"SSC_FRACTION_{i+1}"]
            output["Fraction concentrations"][f"Fraction {i+1}"] = self._parse_format(fraction, variable="Concentration", unit="kg/m³")
        for i in range(self.n_layers):
            layer = initial[f"BED_THICKNESS_LAYER_{i+1}"]
            thickness = self._parse_format(layer, variable="Layer thickness", unit="m")
            layer = initial[f"BED_DISTRIBUTION_LAYER_{i+1}"]
            distribution = {}
            for j in range(self.n_fractions):
                fraction = layer[f"BED_DISTRIBUTION_FRACTION_{j+1}"]
                if fraction["format"] == 0:
                    distribution[f"Fraction {j+1}"] = str(np.round(fraction["constant_value"], 2)) + "%"
                elif fraction["format"] == 2:
                    distribution[f"Fraction {j+1}"] = "Varying in space"
            output["Layer specifications"][f"Layer {i+1}"] = {"Layer thickness": thickness,
                                                             "Fraction distribution": distribution}
        return output

    def _parse_boundaries(self):
        BC = self.MT["BOUNDARY_CONDITIONS"]
        for key in self.boundaries.keys():
            boundary = BC[key]
            if len(boundary.keys()) == 0:
                continue
            self.boundaries[key]["Mud Transport Information"] = {}
            for f in range(self.n_fractions):
                fraction = boundary[f"SSC_FRACTION_{f+1}"]
                tmp = {}
                if fraction["type"] == 1:
                    tmp = {"Type": "Land boundary"}
                elif fraction["type"] == 3:
                    tmp = {"Type": "Zero gradient"}
                elif fraction["type"] == 2:
                    tmp = {"Type": "Specified concentration",
                            **self._parse_format(fraction, variable="Concentration", unit="kg/m³"),
                            "Type of soft start": "Linear variation" if fraction["type_of_soft_start"] == 1 else "Sine variation"}
                    if fraction["format"] == 1:
                        tmp["Type of interpolation in time"] = "Linear" if fraction["type_of_time_interpolation"] == 1 else "Piecewise cubic"
                    if fraction["format"] == 3:
                        tmp["Type of interpolation in time"] = "Linear" if fraction["type_of_time_interpolation"] == 1 else "Piecewise cubic"
                        tmp["Type of interpolation in space"] = "Normal" if fraction["type_of_space_interpolation"] == 1 else "Reverse order"
                self.boundaries[key]["Mud Transport Information"][f"Fraction {f+1}"] = tmp
                    
    def _parse_morphology(self):
        morphology = self.MT["MORPHOLOGY"]["MODEL_DEFINITION"]
        if morphology["include_morphological_update"]:
            if morphology["include_speedup"]:
                return {"Morhpological calculations": "Included with speedup factor",
                        **self._parse_format(morphology["SPEEDUP_FACTOR"], variable="Speedup factor", unit="")}
            return "Morphological calculations included without applying speedup factor"
        return "Morphological calculations not included"
...
class PFS_Parser:
    def __init__(self, fname):
        ...
        if "mode_of_mud_transport_module" in self.modules:
            self.mud_transport = PFS_Mud_Transport(self.pfs["MUD_TRANSPORT_MODULE"], self.boundaries, self.sources)
            self.sources = self.mud_transport.sources
            self.boundaries = self.mud_transport.boundaries
    ...
    def parse(self):
        ...
        if "mode_of_mud_transport_module" in self.modules:
            output["Mud Transport"] = self.mud_transport.attributes
...
fname = "full_benthic_no_gui.m3fm"
print(fname)
pfs = PFS_Parser(fname)
print(pfs.to_excel("full_benthic_no_gui.m3fm.xlsx"))